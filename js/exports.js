...
function toggleSidebar(){S.sidebarOpen=!S.sidebarOpen;var sb=document.querySelector('.sidebar');var ov=document.getElementById('sidebar-overlay');var hb=document.getElementById('hamburger-btn');if(sb)sb.classList.toggle('open',S.sidebarOpen);if(ov)ov.classList.toggle('show',S.sidebarOpen);if(hb)hb.classList.toggle('open',S.sidebarOpen);try{if(navigator.vibrate)navigator.vibrate(10);}catch(e){}}
function hapticTap(ms){try{if(navigator.vibrate)navigator.vibrate(ms||15);}catch(e){}}
